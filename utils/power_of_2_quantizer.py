...
def apply_power_of_2_quantization(model: nn.Module, sample_input: torch.Tensor, 
                                 bitwidth: int = 8) -> nn.Module:
    """
    ...
    with torch.no_grad():
        for name, param in model.named_parameters():
            if param.requires_grad and len(param.shape) > 1:  # Skip bias terms
                ...
                print(f"Layer {name}: scale={info['scale']:.6f} ({info['power_of_2_representation']}), "
...
def demonstrate_power_of_2_quantization():
    ...
    for i, tensor in enumerate(test_tensors):
        ...
        if info['exponent'] > 0:
            ...
            print(f"Hardware implementation: x (no shift needed)")
...
class MultiBitwidthPowerOf2Quantizer:
    ...
    def quantize_weights(self, tensor: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        info = {
            ...
            'power_of_2': f"2^(-{exponent})" if exponent > 0 else f"2^({-exponent})" if exponent < 0 else "1",
